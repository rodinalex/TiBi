...
class DataModel(UserDict):
    ...
    def __setitem__(self, key, val):
        ...
        value is different from the previous value. Signal emission is suppressed
        if __setitem__ is a part of a multi-field update.
...
class AlwaysNotifyDataModel(UserDict):
    ...
    Extends Python's UserDict to add signal emission whenever __setitem__ is called.
    This allows UI components to reactively respond when there are update calls.
    """
    ...
    def __setitem__(self, key, val):
        """
        Set a key-value pair in the dictionary and emit updated signal.
    ...
    def update(self, d):
        """
        Update multiple dictionary values and emit only a single signal.
