...
class DataModel(UserDict):
    ...
    def __setitem__(self, key, val):
        """
        Set a key-value pair in the dictionary and emit a signal if changed.

        This method overrides the dictionary's __setitem__ to add signal
        when the value for a key changes. It only emits the signal if the new
        value is different from the previous value. Signal emission is
        suppressed if __setitem__ is a part of a multi-field update.
    ...
    def update(self, d: dict) -> None:
        """
        Update multiple dictionary values and emit a signal if any changed.
        ...
        ----------
            d : dict
                Dictionary containing the new key-value pairs to update
        """
